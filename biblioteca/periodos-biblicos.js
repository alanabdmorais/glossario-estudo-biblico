...
const periodosBiblicos = {
  ...
  "era-monarquia-unida": {
    ...
    inicio: "~1050 a.C.",
    fim: "~930 a.C.",
    livros: ["1Sm", "2Sm", "1Rs", "1Cr", "2Cr", "Jó", "Sl", "Pv", "Ec", "Ct"],
    lideres: [
...
function buscarPeriodoPorLivroId(livroId) {
  // Extrai a sigla (com o dígito inicial, se houver, ex: "1co", "2sm")
  // parando no primeiro dígito de CAPÍTULO/VERSÍCULO. O split antigo por
  // /[0-9.]/ cortava também o dígito inicial de livros como "1co3",
  // resultando em sigla vazia. Corrigido pra casar "1?letras" no início.
  const m = livroId.match(/^(\d?[a-zçãéíóúàâêô]+)/i);
  const sigla = m ? m[1] : '';
  const siglasMap = {
