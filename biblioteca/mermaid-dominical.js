// ============================================
// DIAGRAMAS MERMAID
// ESCOLA DOMINICAL
// ============================================
// Cada entrada é uma string Mermaid válida
// pronta para ser renderizada via mermaid.js
// ============================================

window.bibliotecaMermaid = window.bibliotecaMermaid || {};

window.bibliotecaMermaid["2T2026-licao13"] = `mindmap
  root((Discernimento\\ndo Cristão))
    Necessidade
      Numerosos ensinos
        Diversidade de doutrinas
        Risco de confusão
      Advertência bíblica
        Examinar tudo
        Provar os espíritos
      Proteção do rebanho
        Liderança vigilante
        Unidade na fé
    Fontes
      Escrituras Sagradas
        Padrão absoluto
        Familiaridade com a Palavra
      Espírito Santo
        Guia em toda verdade
        Dom de discernir espíritos
      Maturidade cristã
        Sentidos exercitados
        Caminhada de fé obediente
    Prática
      Julgar corretamente
        Sem precipitação
        Com base na Palavra
      Cuidado com emoções
        Coração é enganoso
        Razão iluminada
      Obediência à verdade
        Praticantes da Palavra
        Andar na luz
    Falsos Líderes
      Discernir caráter
        Fruto do Espírito
        Vida de oração
      Discernir motivações
        Honrar a Cristo
        Servir a todos
      Confiança na Palavra
        Consistência doutrinária
        Rejeitar o inconsistente
`;

window.bibliotecaMermaid["3T2026-licao1"] = `mindmap
  root((O Livro\\nde Juízes))
    Josué e a Conquista
      Conquista da Terra Prometida
        Esforço e bom ânimo
        Travessia do Jordão
      Deus é o Conquistador
        Fidelidade exclusiva
        Segredo do êxito: obediência
      A morte de Josué
        Vazio de liderança
        Teocracia sem sucessor
    O Livro de Juízes
      Uma geração desorientada
        Sem liderança central
        Cada um fazia o que era reto
      Os Juízes
        Levantados por Deus
        Maiores: Otniel, Eúde, Débora, Gideão, Jefté, Sansão
        Menores: Sangar, Tola, Jair, Ibsã, Elom, Abdom
      Heróis, porém falhos
        Limitações humanas
        Só o Senhor é o verdadeiro Libertador
    Mensagem para Hoje
      Cada um fazia o que parecia certo
        Ausência de liderança
        Valor da autoridade
        Relativismo moral
      Ciclo da libertação
        Infidelidade
        Opressão
        Arrependimento
        Libertação
      No poder do Espírito
        Não é habilidade natural
        Espírito do Senhor se apoderou de
`;

window.bibliotecaMermaid["3T2026-licao2"] = `mindmap
  root((Fidelidade\\na Deus))
    Entre Êxitos e Fracassos
      O começo promissor de Judá
        Consulta ao Senhor
        Tribo escolhida primeiro
      Força divina e união fraterna
        Vitória não é força militar
        Judá e Simeão se unem
      Conquista parcial
        Carros de ferro
        Falta de fé e coragem
    O Anjo do Senhor Repreende
      Deus fala
        Teofania em Boquim
        Questão de vontade, não capacidade
      A desobediência do povo
        Aliança proibida com cananeus
        Por que fizestes isso?
      Choro e remorso
        Sacrifícios oferecidos
        Sem mudança real de atitude
    Vivendo Entre Ídolos
      Uma geração rebelde
        Não conhecia o Senhor
        Esquecimento das origens
      O pecado da idolatria
        Baal e Astarote
        Ritos lascivos e sacrifícios humanos
      Contaminação e sincretismo
        Ciclo de pecado e livramento
        Juízo divino e misericórdia
      Mantendo a fidelidade hoje
        Ídolos modernos
        Fugir da idolatria
`;

window.bibliotecaMermaid["3T2026-licao4"] = `mindmap
  root((Eúde e Sangar\\nDeus Usa os\\nImprováveis))
    Eúde: Deus Usa os Improváveis
      Uma derrota amarga
        Israel volta a pecar
        18 anos sob Eglom
      Um libertador improvável
        Homem canhoto
        Deus usa o inesperado
      Uma grande vitória
        Plano estratégico
        Confiança no Senhor
    Sangar: Deus Usa o Que Temos à Disposição
      Um juiz desconhecido
        Aguilhada de bois
        80 anos de paz
      Um nome estrangeiro
        Origem não hebraica
        Deus usa qualquer um
      Uma arma diferente
        Ferramenta de trabalho
        Usar o que temos à mão
    A Questão da Violência
      Um Deus injusto?
        Padrão objetivo de justiça
        Deus é o referencial
      Pessoas inocentes
        Ninguém é inocente
        Juízo por idolatria
      Revelação progressiva
        Textos descritivos, não prescritivos
        Plenitude em Cristo
`;
